...
board_links = get_board_links(config)
logger.info(f"Board links configured: {list(board_links.keys())}")
...
def get_boards():
    """Return Monday.com board links."""
    try:
        board_links = get_stored_board_links()
        return jsonify({
            'status': 'success',
            'boards': board_links
        })
